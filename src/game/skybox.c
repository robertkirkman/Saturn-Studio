...
SkyboxTexture cloud_floor_skybox_ptrlist = { SKYTEX("cloud_floor") };
...
Gfx *create_skybox_facing_camera(s8 player, s8 background, f32 fov,
    ...
    if ((gCurrLevelNum == LEVEL_SA && colorIndex == 1 || autoChroma) && use_color_background) {
        ...
        sSkyboxColors[colorIndex][1] = chromaColor.green[1];
        sSkyboxColors[colorIndex][2] = chromaColor.blue[1];
